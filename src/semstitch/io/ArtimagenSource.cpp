...
// --- Суровый шум из библиотеки не нужен в консоли
void ArtimagenSource::msgCallback(t_message* m) {
    if (!m) return;
    // Показываем только редкие/важные события
    switch (m->message) {
        ...
        case AIG_MSG_SAVING:
            // Изредка можно подсветить этапы
            static int skip = 0;
            if ((skip++ % 50) == 0) {
...
ArtimagenSource::ArtimagenSource(const std::string& /*luaSceneFile*/) {
    // Подкручиваем потоки FFTW (если собрано с тредами) и callback логов
    if (AIGApp) {
        AIGApp->set_message_call_back(&ArtimagenSource::msgCallback);
        // На всякий: не навредит, если тредов нет
        AIGApp->set_number_of_threads(std::max(2u, std::thread::hardware_concurrency()));
    }

    // --- Упростим sample (ускоряет старт). Вернёшь выше — будет «дороже».
    gcDef_.sizex = 3000;   // nm — реальный размер области (не критично)
    gcDef_.sizey = 3000;
    gcDef_.number_of_grains = 80;   // было 300 — долго
    gcDef_.grain_min_size   = 15;
    gcDef_.grain_max_size   = 60;
    gcDef_.ee_coefficient   = 0.0f; // эффект края зерна (оставим по умолчанию)
    gcDef_.fs_density       = 0.0f; // тонкая структура off

    // --- Параметры итогового кадра
    imgDef_.sizex = 384;   // 512 → 384, чтобы стартовал быстрее. Можно вернуть.
    imgDef_.sizey = 384;
    ...
    // пучок (PSF) — чем меньше sigma, тем быстрее свёртка
    ...
    // --- Создаём выборку и первый кадр (для прогрева планов FFT)
    ...
    // сразу освобождаем: мы всё равно будем перегенерировать в next()
...
std::optional<Frame> ArtimagenSource::next() {
    // Генерируем новый CImage на каждую итерацию
    if (image_) { aig_destroy_image(image_); image_ = nullptr; }

    image_ = generate_standard_image(sample_, &imgDef_);
    if (!image_) return std::nullopt;

    // Достаём double* буфер из CImage и конвертируем в 8-bit
    ...
    const double* buf = img->get_buffer(); // диапазон [0..1]
    scratch_.resize(N);
    ...
    // CImage больше не нужен — освобождаем сразу
    aig_destroy_image(image_);
    image_ = nullptr;
